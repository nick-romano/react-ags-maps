...
var animatedLayer = esri_loader_1.loadModules([
    "esri/Map",
    ...
    var CustomLayerView2D = BaseLayerViewGL2D.createSubclass({
        ...
        constructor: function () {
            ...
            this.transform = gl_matrix_1.mat3.create();
            this.translationToCenter = gl_matrix_1.vec2.create();
            this.screenTranslation = gl_matrix_1.vec2.create();
            // Geometrical transformations whose only a few elements
            // must be updated per frame. Those elements are marked
            // with NaN.
            this.display = gl_matrix_1.mat3.fromValues(NaN, 0, 0, 0, NaN, 0, -1, 1, 1);
            this.screenScaling = gl_matrix_1.vec3.fromValues(NaN, NaN, 1);
        ...
        attach: function () {
            ...
            this.centerAtLastUpdate = gl_matrix_1.vec2.fromValues(this.view.state.center[0], this.view.state.center[1]);
        },
        ...
        render: function (renderParameters) {
            ...
            gl_matrix_1.mat3.identity(this.transform);
            this.screenTranslation[0] = (state.pixelRatio * state.size[0]) / 2;
            this.screenTranslation[1] = (state.pixelRatio * state.size[1]) / 2;
            gl_matrix_1.mat3.translate(this.transform, this.transform, this.screenTranslation);
            gl_matrix_1.mat3.rotate(this.transform, this.transform, (Math.PI * state.rotation) / 180);
            this.screenScaling[0] = state.pixelRatio / state.resolution;
            this.screenScaling[1] = -state.pixelRatio / state.resolution;
            gl_matrix_1.mat3.scale(this.transform, this.transform, this.screenScaling);
            gl_matrix_1.mat3.translate(this.transform, this.transform, this.translationToCenter);
        ...
        hitTest: function (x, y) {
            ...
            var distances = this.layer.graphics.map(function (graphic) {
                ...
                return Math.sqrt((graphicPoint.x - x) * (graphicPoint.x - x) +
        ...
        updatePositions: function (renderParameters) {
            ...
            if (!stationary) {
                gl_matrix_1.vec2.sub(this.translationToCenter, this.centerAtLastUpdate, state.center);
                this.requestRender();
                return;
            }
